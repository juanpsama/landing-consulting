// src/pages/api/vcard/[miembro].ts
import type { APIRoute } from 'astro';
import { teamMembers } from '../../../data/links';

export const GET: APIRoute = async ({ params, request }) => {
    const memberSlug = params.miembro || "";
    ...
    console.log(`[vCard API] Generating contact for: "${memberSlug}" on origin: ${currentOrigin}`);

    if (!member) {
        return new Response("Contacto no encontrado / Contact not found", { status: 404 });
    }
